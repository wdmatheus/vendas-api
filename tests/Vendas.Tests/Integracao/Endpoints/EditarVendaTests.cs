...
public sealed class EditarVendaTests : IClassFixture<ApiFixture>
{
    private readonly ApiFixture _apiFixture;

    public EditarVendaTests(ApiFixture apiFixture) => _apiFixture = apiFixture;
    ...
    public async Task DeveEditarVenda()
    {
        ...
        using var request = new HttpRequestMessage(HttpMethod.Put, $"v1/vendas/{venda.Id}");
    ...
    {
        //Arrange
        var payload = RequestsFakerFactory.BuildEditarVendaRequest(1);
        
        using var request = new HttpRequestMessage(HttpMethod.Put, $"v1/vendas/{Guid.NewGuid()}");
        request.Content = new StringContent(
            JsonSerializer.Serialize(payload, JsonHelper.JsonSerializerOptions),
            Encoding.UTF8,
            MediaTypeNames.Application.Json
        );
        var erroEsperado = VendaMensagens.VendaNaoExiste;
